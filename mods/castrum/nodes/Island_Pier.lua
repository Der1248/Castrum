...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname == "island_pier" then
        for k, v in pairs(fields) do
            if v == "del" then
                minetest.place_schematic({x=171, y=6, z=38}, minetest.get_modpath("castrum").."/schematics/Island_Pier/Island_Pier_0.mts","0") 
            ...
            elseif v == "Upgrade" or v == "Build" then
                ...
                if tonumber(level) < 7 and inv:contains_item("main", "default:wood 100") and tonumber(fountain) > 3 then
                    Item_Remove2(player, "main", "default:wood 100")
                    upgrade = true
				elseif tonumber(level) < 7 and inv:contains_item("main", "default:wood 100") then
                    txt = "build island fountain first"
				elseif tonumber(level) == 7 and inv:contains_item("main", "default:junglewood 500") and tonumber(fountain) > 4 and tonumber(chapter) > 2 then
                    Item_Remove2(player, "main", "default:junglewood 500")
                    upgrade = true
				elseif tonumber(level) == 7 and inv:contains_item("main", "default:junglewood 500") and tonumber(chapter) > 2 then
                    txt = "upgrade island fountain to lv.2 first"
				elseif tonumber(level) == 7 and inv:contains_item("main", "default:junglewood 500") then
                ...
                if (tonumber(level)) < 8 and upgrade or buildings_costs == false then
                    minetest.place_schematic({x=171, y=6, z=38}, minetest.get_modpath("castrum").."/schematics/Island_Pier/Island_Pier_"..(tonumber(level)+1)..".mts","0")
                    file = io.open(minetest.get_worldpath().."/SAVE/Island_Pier.txt", "w")
