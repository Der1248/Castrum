...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname == "cannon1" then
        for k, v in pairs(fields) do
            if v == "del" then
                minetest.place_schematic({x=169, y=9, z=42}, minetest.get_modpath("castrum").."/schematics/Cannon1/Cannon1_0.mts","0") 
            ...
            elseif v == "Upgrade" or v == "Build" then
                ...
                if (tonumber(level)) < 2 and upgrade or buildings_costs == false then
                    minetest.place_schematic({x=169, y=9, z=42}, minetest.get_modpath("castrum").."/schematics/Cannon1/Cannon1_"..(tonumber(level)+1)..".mts","0")
                    file = io.open(minetest.get_worldpath().."/SAVE/Cannon1.txt", "w")
