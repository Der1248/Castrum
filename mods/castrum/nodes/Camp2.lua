...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname == "camp2" then
        for k, v in pairs(fields) do
            if v == "del" then
                minetest.place_schematic({x=-67, y=9, z=-74}, minetest.get_modpath("castrum").."/schematics/Camp2/Camp2_0.mts","0")
            ...
            elseif v == "Upgrade" or v == "Build" then
                ...
                if (tonumber(level)) < 1 and upgrade or buildings_costs == false then
                    minetest.place_schematic({x=-67, y=9, z=-74}, minetest.get_modpath("castrum").."/schematics/Camp2/Camp2_"..(tonumber(level)+1)..".mts","0")
                    file = io.open(minetest.get_worldpath().."/SAVE/Camp2.txt", "w")
