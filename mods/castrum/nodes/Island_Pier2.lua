...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname == "island_pier2" then
        for k, v in pairs(fields) do
            if v == "del" then
                minetest.place_schematic({x=176, y=6, z=42}, minetest.get_modpath("castrum").."/schematics/Island_Pier2/Island_Pier2_0.mts","0") 
            ...
            elseif v == "Upgrade" or v == "Build" then
                ...
                if tonumber(level) < 3 and inv:contains_item("main", "default:wood 100") and tonumber(fountain) > 3 then
                    Item_Remove2(player, "main", "default:wood 100")
                    upgrade = true
				elseif tonumber(level) < 3 and inv:contains_item("main", "default:wood 100") then
                    txt = "build island fountain first"
				elseif tonumber(level) == 3 and inv:contains_item("main", "default:junglewood 200") and tonumber(fountain) > 4 and tonumber(chapter) > 2 then
                    Item_Remove2(player, "main", "default:junglewood 200")
                    upgrade = true
				elseif tonumber(level) == 3 and inv:contains_item("main", "default:junglewood 200") and tonumber(chapter) > 2 then
                    txt = "upgrade island fountain to lv.2 first"
				elseif tonumber(level) == 3 and inv:contains_item("main", "default:junglewood 200") then
                ...
                if (tonumber(level)) < 4 and upgrade or buildings_costs == false then
                    minetest.place_schematic({x=176, y=6, z=42}, minetest.get_modpath("castrum").."/schematics/Island_Pier2/Island_Pier2_"..(tonumber(level)+1)..".mts","0")
                    file = io.open(minetest.get_worldpath().."/SAVE/Island_Pier2.txt", "w")
